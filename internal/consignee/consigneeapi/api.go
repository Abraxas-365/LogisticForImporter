...
func SetupRoutes(app *fiber.App, service *consignee.Service, authMiddleware *lucia.AuthMiddleware[*auth.AuthUser]) {
	...
	app.Post("/consignatarios", authMiddleware.RequireAuth(), func(c *fiber.Ctx) error {
		...
		newConsignatarioInput := struct {
			DocumentType   consignee.DocumentType `json:"document_type"`        // Document type (RUC 10, RUC 20, DNI, CI, Passport)
		...
		newConsignatario := consignee.Consignee{
	...
	app.Put("/consignatarios/:id", authMiddleware.RequireAuth(), func(c *fiber.Ctx) error {
		...
		updatedConsignatario := consignee.Consignee{
			ID:             id,
